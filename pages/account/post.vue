                ...
                <div  v-if="list.length > 0"  class="list">
                    <div v-for="(item,index) in list" :key="index" class="item">
                        <div class="header">
                            ...
                            <h2 class="title"  @click="goPath(`/post/${item.id}`)">{{item.title}}</h2>
                            <div class="cover-desc">
                                <div @click="goPath(`/post/${item.id}`)" class="desc">
                                    {{item.content}}
                                </div>
                                <div @click="goPath(`/post/${item.id}`)" v-if="item.images" class="cover">
                                    <img :src="item.images[0] | resetImage(140,75)" alt="" srcset="">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="footer">
                            ...
                            <div class="action">
                                ...
                                <div @click="goPath(`/post/edit?id=${item.id}`)" class="action-btn">
                                    <a-icon type="edit" />
                                    <span>编辑</span>
                                </div>
                            </div>
...
export default {
    ...
    methods:{
        ...
        async getList(){
            const res = await this.$axios.get(api.getAccountPostList,{params: this.query})
            if (res.code != 1) {
                this.$message.error(
                    res.message,
                    3
